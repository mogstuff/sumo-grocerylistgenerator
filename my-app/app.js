...
document.addEventListener("apiReady", function(){
  /*  document.getElementById("try-now-get").setAttribute("style", "display:block");
    ...
    // Get Recipes
     window.app.getRecipes = function () {
        window.df.apis.db.getRecords({table_name: "Recipes"}, function (response) {
            //Do something with the data;
           document.getElementById("recipes").innerHTML = JSON.stringify(response);

        }, function(response) {
            document.getElementById("recipes").innerHTML = window.app.getErrorString(response);
        });
    };
    
    
    // Get Ingredients
      window.app.getIngredients = function () {
        window.df.apis.db.getRecords({table_name: "Ingredients"}, function (response) {
            //Do something with the data;
           document.getElementById("ingredients").innerHTML = JSON.stringify(response);

        }, function(response) {
            document.getElementById("ingredients").innerHTML = window.app.getErrorString(response);
        });
    };
